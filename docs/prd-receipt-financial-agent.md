...
- Relative periods (`last_week`, `last_month`, and `last_quarter`) are the default tool inputs; explicit start/end dates are also supported for uncommon audits.
...
- `last_quarter` means the previous complete calendar quarter from its first day at 00:00 through the current quarter’s first day at 00:00.
