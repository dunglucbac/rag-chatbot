...
# Prevent ONNX Runtime from persisting its telemetry identifier beside worker
# source files when the extractor is imported.
os.environ.setdefault("ORT_DISABLE_TELEMETRY", "1")

from huggingface_hub import snapshot_download

from ..paths import get_project_base_directory
...
class TextRecognizer:
    def __init__(self, model_dir=None, device_id: int | None = None):
        vietocr_root = Path(get_project_base_directory()) / "vietocr"
        config = Cfg.load_config_from_file(vietocr_root / "config" / "base.yml")
        config.update(
            Cfg.load_config_from_file(vietocr_root / "config" / "vgg-seq2seq.yml")
        )
        config["weights"] = str(vietocr_root / "weight" / "vgg_seq2seq.pth")
