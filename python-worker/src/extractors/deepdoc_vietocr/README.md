...
> **Ghi chú tích hợp:** Bản vendored này chỉ dùng cho worker runtime. Các script
> demo và thư mục đầu ra `log/`, `img/` đã được loại bỏ. Worker gọi
> `DeepDocPipeline.extract()` qua `DeepDocVietOcrExtractor` và nhận Markdown
> trong bộ nhớ, không tạo file kết quả.
