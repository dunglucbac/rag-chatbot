...
> **Integration note:** This vendored copy is runtime-only. Demo scripts and the
> generated `log/` and `img/` directories have been removed. The worker calls
> `DeepDocPipeline.extract()` through `DeepDocVietOcrExtractor` and receives
> Markdown in memory without creating result files.
